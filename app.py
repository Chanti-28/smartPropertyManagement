...
#     # Visibility + contact (kept for future, not enforced in queries)
...
#         flash(
#             "Thank you! Your enquiry has been submitted. The owner will contact you "
#             "soon.",
#             "success",
#         )
...
#     Endpoint name is 'index' so url_for('index') -> /dashboard.
#     """
#     user = current_user()
#     if not user:
...
# def init_db():
#     """Create or update DB tables via: flask --app app.py init-db"""
...
class Property(db.Model):
    ...
    city = db.Column(db.String(100), nullable=False)
    short_address = db.Column(db.String(255))  # public-friendly location
    ...
    # Visibility + contact (we keep but don’t force filter)
    is_public = db.Column(db.Boolean, default=True)
    contact_phone = db.Column(db.String(50))
    contact_email = db.Column(db.String(120))
...
@app.route("/")
def home():
    """Public homepage: show all properties."""
    props = Property.query.order_by(Property.created_at.desc()).all()
    return render_template("public_list.html", properties=props)
